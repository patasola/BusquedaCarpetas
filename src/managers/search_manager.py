...
class SearchManager:
    def __init__(self, cache_manager, search_engine, progress_manager, ui_manager):
    ...
    def _validar_busqueda(self, criterio):
        ...
        if not self.cache_manager.ruta_base:
            self.ui_manager.mostrar_advertencia("Seleccione una ruta de búsqueda primero")
            return False
    ...
    def _preparar_busqueda(self, criterio):
        ...
        self.ui_manager.limpiar_resultados()
        self.ui_manager.actualizar_estado("Buscando...")
    
    def _buscar_en_cache(self, criterio):
        """Intenta búsqueda en cache primero"""
        if not self.cache_manager.cache.valido:
            self.ui_manager.actualizar_estado("Cache no disponible. Construyendo cache...")
            self.ui_manager.construir_cache()
            return True
        ...
        if resultados is not None:
            if len(resultados) > 0:
                ...
                self.ui_manager.actualizar_estado(
                    f"Cache sin resultados ({tiempo_cache:.3f}s), buscando tradicionalmente..."
                )
                return False
        
        self.ui_manager.actualizar_estado("Error en cache, buscando en directorios...")
        return False
    
    def _iniciar_busqueda_tradicional(self, criterio):
        """Inicia búsqueda tradicional en thread separado"""
        self.ui_manager.actualizar_estado("Búsqueda tradicional iniciada... 0%")
        ...
        def callback_progreso(procesados, total):
            if total > 0:
                ...
                if porcentaje % 5 == 0 or procesados >= total:
                    ...
                    self.ui_manager.actualizar_estado_async(mensaje)
